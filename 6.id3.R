...
ig_cat <- function(data, feature, target) {
  ...
  dd_data <- data %>% group_by_at(feature) %>% summarise(e = entropy(get(target)), n = length(get(target)))
...
f2 <- function() {
  ...
  tree <- Node$new("Class")
  tryCatch(train_id3(tree, my_data, ordered_df, 1), error = function(e) {})
  #print(tree, "feature", "obsCount")

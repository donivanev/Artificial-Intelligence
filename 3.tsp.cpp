...
void generateGraph(std::map<T,std::vector<int>> &simpleGraph, size_t N) {
    ...
    // p1.first -> (first,second) -> (x1,y1)
    // p2.first -> (first,second) -> (x2,y2)
    ...
    // for (const auto &point1: graph) {
    //     std::cout << "(" << point1.first.first << ", "<< point1.first.second << ") -> ";
    //     for (size_t i = 0; i < point1.second.size(); i++) {
    //         std::cout << point1.second[i] << " ";
    //     }
    //     std::cout << std::endl;
    // }
...
void calculateCities(std::map<int,std::vector<int>> &result) {
    ...
    // p1.second -> (first,second) -> (x1,y1)
    // p2.second -> (first,second) -> (x2,y2)
